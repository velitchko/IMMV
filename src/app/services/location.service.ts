...
export class LocationService {
  ...
  /**
   * Returns all locations we have in the service
   * @return this.locations - array of locations we currently have
   */
  ...
  /**
   * Performs HTTP GET to recieve all locations from the DB
   * @return promise - either resolve and return all locations or reject with error
   */
        ...
        (response: any) => {
          if(response.data) {
            for(let l of response.data) {
              this.locations.push(this.getAsLocation(l));
            }
            resolve(this.locations);
          } else {
            reject('Could not parse locations');
          }
        }
      );
    });
  ...
  getLocation(id: string): Promise<any> {
    let promise = new Promise((resolve, reject) => {
      this.http.get(environment.API_URL + 'locations/' + id).subscribe(
       (response: any) => {
         if(response.data) {
           resolve(this.getAsLocation(response.data));
         } else {
  ...
  /**
   * Query the database for locations matching a given name
   @param name - the name of the location we are looking for
   @return promise - promise with resolve and reject
   */
  findLocation(name: string): Promise<any> {
    // TODO: Implement this
