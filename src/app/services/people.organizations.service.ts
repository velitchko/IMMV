...
export class PersonOrganizationService {
  ...
  /**
   * Returns all organizations we have in the service
   * @return this.organizations - array of organizations we currently have
   */
  getOrganizations(): PersonOrganization[] {
    return this.organizations;
  }

   /**
   * Returns all people we have in the service
   * @return this.people - array of organizations we currently have
   */
  getPeople(): PersonOrganization[] {
    return this.people;
  }

  /**
   * Performs HTTP GET to recieve all organizations from the DB
   * @return promise - either resolve and return all organizations or reject with error
   */
  getAllPeopleOrganizations(): Promise<any> {
    let promise = new Promise( (resolve, reject) => {
      // objectType property = ['Person', 'Organization']
      this.http.get(environment.API_URL + 'peopleorganizations/').subscribe(
        (response: any) => {
          if(response.data) {
            for(let o of response.data) {
              let personOrganization = this.getAsPersonOrganization(o);
            ...
            reject('Could not parse people/organizations');
          }
        });
    });
    return promise;
   ...
   * @return promise - either resolve and return all organizations or reject with error
   */
  getPersonOrganization(id: string): Promise<any> {
    let promise = new Promise( (resolve, reject) => {
      // objectType property = ['Person', 'Organization']
      this.http.get(environment.API_URL + 'peopleorganizations/' + id).subscribe(
        (response: any) => {
          if(response.data) {
            resolve(this.getAsPersonOrganization(response.data));
          } else {
            reject('Could not parse people/organizations');
          }
        });
  ...
  /**
   * Query the database for organizations matching a given name
   @param name - the name of the organizations we are looking for
   @return promise - promise with resolve and reject
   */
  findOrganization(name: string): Promise<any> {
    // TODO: Implement
  ...
  /**
   * Query the database for people matching a given name
   @param name - the name of the people we are looking for
   @return promise - promise with resolve and reject
   */
  findPerson(name: string): Promise<any> {
    return new Promise<any>((resolve, reject) => {});
  }
}
